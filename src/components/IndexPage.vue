...
export default {
    ...
    update() {
      ...
      this.makeTree_out(this.input_val)
      this.makeTree_in(this.input_val)
      this.getData()
    }, 
    ...
    async getData () {
        ...
        this.total_in = this.txes.total_in
        },  

    async makeTree_out (address) {

    async function getData(address, block)  {
      if (!block) {
        const response = await dataService.fetchData(address, 'out');
        return response.data 
        }
      else 
      {
        const response = await dataService.fetchData(address, 'out', block)
        return response.data 
        }
  ...
  await nodedata.tx_out.forEach(async item =>  {
    ...
    //pan & zoom------------------------------------------------------------------------------
      ...
      const height = Math.max(document.documentElement.clientHeight,document.body.clientHeight)*2/3
...
// for incoming txes
//----------------------------------------------------------------------------------------------------------------------------------------
    async makeTree_in(address)
    {
      
    async function getData(address, block)  {
      if (!block) {
        const response = await dataService.fetchData(address, 'in');
        return response.data 
        }
      else 
      {
        const response = await dataService.fetchData(address, 'in', block)
        return response.data 
        }
    ...
    async function addN(selected, item){
      ...
      const nodedata1 =  await getData(newNode.data._id, item.lastBlock) //look for txes from given address and after given block (to make sure we look for descending txes only)

      if (nodedata1.total_in[0] != null) 
        {
  ...
  var root = d3.hierarchy(newNode)
      if (nodedata.total_in[0] != null) 
        {
          root.data.tx_in = nodedata.total_in[0].links,
          root.data.value_in = nodedata.total_in[0].value
        }
      root.data.children = nodedata.tx_in
  if (nodedata.aliases[0] != null){root.data.alias = nodedata.aliases[0].tokenName}
  root.data.type = 'in'
  //add each linked address as a child node
  await nodedata.tx_in.forEach(async item =>  {
    ...
    function update(source) {
      ...
      const duration = d3.event && d3.event.altKey ? 2500 : 250;
      const nodes = root.descendants().reverse();
      root.sort(function(a, b) { return - b.data.value_out + a.data.value_out; }); //sort by incoming value
...
//taking address as an argument, looking through all the tree data for duplicates. when duplicate found, return it 
      async function dupe(item) {
        let result = 0;
                        await root.descendants().forEach(s=>{
                          if (item._id == s.data._id) {
                              if (item.lastBlock > s.data.lastBlock) {s.data.lastBlock = item.lastBlock; update(s)}
                            result = s;
                          }   
                        }) 
        return result
